...
Three rendering decisions were made in that conversion. Every body paragraph in the supplied document is italicised and none are upright, so the italics carry no emphasis and the web rendering sets body prose upright, keeping italics for captions and the provenance note. The document also reused one image for both Figure 1.2 and Figure 1.4. Figure 1.4 is no longer that duplicate: it is redrawn in `dist/chapter/figure-1-4.svg` from a photograph of the printed page, so the chapter's own plot is shown rather than a stand-in for it. The redrawing keeps the printed figure's furniture — outer frame, legend strip, the category axis on the zero line, open squares against filled diamonds — and its series are read off that plot by eye to about half a grid division. They are not the author's data; no data file for them exists here, and the caption says so. `scripts/figure-1-4.mjs` holds the traced points as editable arrays and regenerates the asset with `node scripts/figure-1-4.mjs`; it is the only generated file in `dist`. The remaining figures are still the reconstruction's indicative redrawings, as supplied. The reconstruction labels its figure redrawings as indicative, and they are not used as measured data in this lab.

The third concerns the equations. The conversion left all forty displayed equations in the ASCII the document produced — `X_t`, `^{1/2}`, `_{i,j}`, hyphens for minus signs — set in the interface's sans face, which is source notation rather than mathematics. They are now typeset in HTML: a serif face with real italics for the quantities, upright operators, function names and digits, true sub- and superscripts, proper minus signs, limits set above and below a sum or product that carries both, and the one bivariate system under a single brace. Inline notation in the surrounding prose was converted with them. No formula library is used, so the page still loads nothing at run time; forty equations of this complexity do not need one, and the choice keeps the chapter readable with scripts disabled. Greek letters are left upright because Georgia has no italic Greek and `font-synthesis` is off, so an italic sigma would be served by a fallback face and read as a different symbol from its upright neighbours. Prose mentions of simple quantities — d(q), zeta(q), h(t) — stay in the body face; only displayed mathematics and the notation that was ASCII are typeset.
