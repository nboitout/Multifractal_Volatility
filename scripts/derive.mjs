...
async function main() {
  ...
  if (args.includes('--fixture')) {
    ...
    datasets = [{
      ...
      provenance: {
        ...
        source: 'Generated by scripts/derive.mjs --fixture', kind: 'fixture',
  ...
  for (const {bars, provenance} of datasets) {
    ...
    manifest.push({
      symbol: provenance.symbol, label: provenance.label, kind: provenance.kind,
      resolutionMin: provenance.resolutionMin, from: provenance.from, to: provenance.to, file,
    });
  ...
  writeFileSync(resolve(OUT, 'manifest.json'), JSON.stringify({schema: 1, datasets: manifest}, null, 2));
  console.log(`manifest.json lists ${manifest.length} dataset(s).`);
}
